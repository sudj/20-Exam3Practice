...
def integers(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        random = sequence_of_sequences[k]
        if type(random) is list or tuple:
            for j in range(len(random)):
                if type(random[j]) is int:
                    number = number + [random[j]]
    return number
